...
def train_svm(X_train, y_train):
    ...
    svm_regressor = GridSearchCV(estimator=svr, param_grid =param_grid_svm, cv=3, scoring=scoring_method, verbose=0)
...
def train_gbm(X_train, y_train):
    ...
    gbm_regressor = GridSearchCV(estimator=gbm, param_grid=param_grid_gbm,
                                       cv=3,  scoring=scoring_method, verbose=0)
...
def run_4model(df,features_column, label_column,date_column,tic_column,
    ...
    df_predict_svr = pd.DataFrame(columns=unique_ticker, index=trade_date)
    ...
    for i in range(first_trade_date_index, len(unique_datetime)):
        print(f'curr idx for trade date: {i} ---')
        try:
            ...
            print(f'i, first_trade_date_index, len(unique_datetime) -- {i} {first_trade_date_index} {len(unique_datetime)}')
            ...
            svr_eval = evaluate_model(svr_model, X_test, y_test)
            ...
            y_trade_svr  = svr_model.predict(X_trade)
            ...
            append_return_table(df_predict_svr, unique_datetime, y_trade_svr, trade_tic, current_index=i)
